...
import com.damaha.actionblog.sms.global.RedisConf;
import com.damaha.actionblog.commons.feign.SearchFeignClient;
import com.damaha.actionblog.commons.feign.WebFeignClient;
import com.damaha.actionblog.sms.global.SysConf;
import com.damaha.actionblog.utils.JsonUtils;
import com.damaha.actionblog.utils.RedisUtil;
import com.damaha.actionblog.utils.SpringUtils;
import com.damaha.actionblog.base.enums.ESearchModel;
...
public class BlogListener {
    ...
    @RabbitListener(queues = "action.blog")
        ...
        if (map != null) {
            ...
            try {
                switch (comment) {
                    ...
                    case SysConf.DELETE: {
                        log.info("action-sms处理删除博客: uid:" + uid);
                        updateSearch(map);
                        if (ESearchModel.ES.equals(searchModel)) {
                            // 增加ES索引
                    ...
                    break;
                    default: {
                        log.info("action-sms处理博客兜底方法");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
                log.error("【action-sms】出现异常，请查看action-search是否启动！searchModel: " + searchModel);
            }
        }
    }
